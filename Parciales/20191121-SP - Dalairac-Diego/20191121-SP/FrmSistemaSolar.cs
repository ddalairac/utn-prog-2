...
namespace _20191121_SP
{
    public partial class FrmSistemaSolar : Form
    {
        ...
        {
            string desktop = this.xml.GetDirectoryPath;
            string path = Path.Combine(desktop, FrmSistemaSolar.archivoPlanetas);
            if (this.xml.FileExists(path))
            {
                // Leo mis planetas del archivo xml
                try
                {
                    this.xml.Leer(path, out this.planetas);
                }
                catch (Exception ex)
                {
            ...
            {
                // Asociar Evento
                p.informacionDeAvance += this.DibujarAvancePlaneta;
        ...
        private void btnSimular_Click(object sender, EventArgs e)
        {
            ...
            this.Text = this.EstadoSimulacion(this.btnSimular.Text)
                ? "Detenido :: Diego Dalairac"
                : "Orbitando :: Diego Dalairac";
        ...
        private void FrmSistemaSolar_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            try
            {
                xml.Guardar(path, this.planetas);
            }
            catch (Exception ex) { 
                MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
